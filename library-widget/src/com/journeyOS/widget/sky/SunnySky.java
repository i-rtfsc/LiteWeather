...
public class SunnySky extends BaseSky {
    static final String TAG = SunnySky.class.getSimpleName();
    ...
    public SunnySky(Context context) {
        super(context, false);
    ...
    protected void setSize(int width, int height) {
        ...
        if (this.holders.size() == 0) {
            ...
            for (int i = 0; i < SUNNY_COUNT; i++) {
                final float curSize = maxSize - i * deltaSize * SkyUilts.getRandom(0.9f, 1.1f);
...
//		}
    ...
    public static class SunnyHolder {
        ...
        public SunnyHolder(float x, float y, float w, float h) {
            ...
            this.w = w;
            this.h = h;
            this.curAlpha = SkyUilts.getRandom(minAlpha, maxAlpha);
        }
...
//            curAlpha = Math.max(0f, Math.min(maxAlpha, curAlpha));
            final float delta = SkyUilts.getRandom(0.002f * maxAlpha, 0.005f * maxAlpha);
            if (alphaIsGrowing) {
                curAlpha += delta;
