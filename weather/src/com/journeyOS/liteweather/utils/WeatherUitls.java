...
public class WeatherUitls {
    ...
    public static SkyType convertWeatherType(WeatherData weather) {
        if (weather == null) {
            return SkyType.DEFAULT;
        }
        final boolean isNight = isNight(weather);
        try {
            final int w = Integer.valueOf(weather.now.weatherCode);
            switch (w) {
                case 100:
    ...
    public static boolean isNight(WeatherData weather) {
        boolean enableNight = SpUtils.getInstant().getBoolean(ENABLE_NIGHT, false);
        if (!enableNight) {
            return false;
        }
        ...
        try {
            ...
            WeatherData.DailyEntity todayForecast = null;
            for (WeatherData.DailyEntity forecast : weather.daily) {
            ...
            if (todayForecast != null) {
                ...
                final int srTime = Integer.valueOf(todayForecast.sunRise.replaceAll(":", ""));// 日出
                final int ssTime = Integer.valueOf(todayForecast.sunSet.replaceAll(":", ""));// 日落
